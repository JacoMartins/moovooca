...
class LinhaModel(db.Model):
    ...
    paradas = db.relationship(
        "ParadaModel", back_populates="linha", cascade="all, delete"
    )
    viagens = db.relationship(
        "ViagemModel", back_populates="linha", cascade="all, delete"
    )
    sentidos = db.relationship(
        "SentidoModel", back_populates="linha", cascade="all, delete"
    )
